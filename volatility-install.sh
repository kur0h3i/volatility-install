...
for arg in "$@"; do
    case $arg in
        --force|-f) FORCE=1 ;;
        *) error "Argumento desconocido: $arg. Uso: sudo bash volatility-install.sh [--force]" ;;
    esac
done
...
install_volatility2() {
    ...
    # Comprobar si ya está instalado y funcional
    if [ "$FORCE" -eq 0 ] && [ -f "$VENV_DIR/bin/python" ] && "$VENV_DIR/bin/python" "$INSTALL_DIR/vol.py" --help &>/dev/null; then
        warn "Volatility 2 ya está instalado y funciona → $INSTALL_DIR (usa --force para reinstalar)"
        return 0
    elif [ -d "$INSTALL_DIR" ]; then
        if [ "$FORCE" -eq 1 ]; then
            warn "--force activo: eliminando $INSTALL_DIR para reinstalar"
            rm -rf "$INSTALL_DIR"
        else
            warn "Directorio $INSTALL_DIR existe pero la instalación parece incompleta — continuando..."
        fi
    fi
...
install_volatility3() {
    ...
    # Comprobar si ya está instalado y funcional
    if [ "$FORCE" -eq 0 ] && [ -f "$VENV_DIR/bin/python" ] && "$VENV_DIR/bin/python" "$INSTALL_DIR/vol.py" --help &>/dev/null; then
        warn "Volatility 3 ya está instalado y funciona → $INSTALL_DIR (usa --force para reinstalar)"
        return 0
    elif [ -d "$INSTALL_DIR" ]; then
        if [ "$FORCE" -eq 1 ]; then
            warn "--force activo: eliminando $INSTALL_DIR para reinstalar"
